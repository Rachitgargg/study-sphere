...
import { mockKnowledgeMapData } from '../lib/mockData';

export const KnowledgeMap: React.FC = () => {
  const { documents, setActiveDoc } = useStudySphere();
  const navigate = useNavigate();
  const [selectedNode, setSelectedNode] = useState<string | null>('doc-1');

  const { nodes, links } = mockKnowledgeMapData;
  ...
  // Custom coordinate mappings for mock layout rendering on a 500x350 grid
  const nodeCoords: { [key: string]: { x: number; y: number } } = {
    ...
    // Quantum Concepts
    'c-qubit': { x: 180, y: 320 },
    'c-superpos': { x: 250, y: 330 },
    'c-entangle': { x: 320, y: 310 }
  };
          ...
          <div className="w-full h-full flex items-center justify-center pt-8">
            ...
            >
              ...
              {links.map((link, idx) => {
                ...
                const isHighlight = selectedNode === link.source || selectedNode === link.target;
                return (
                  <line
                    key={idx}
                    x1={sCoord.x}
          ...
          <div className="bg-academic-paper border border-academic-card p-6 rounded-xl min-h-[320px] flex flex-col justify-between gold-glow animate-fade-in">
            <div className="space-y-4">
              ...
              <div className="space-y-2 pt-2">
                <span className="text-[10px] font-mono text-academic-text-muted uppercase tracking-wider block">Connected Nodes</span>
                  ...
                  {connectedNodeIds.map((cId) => {
                    ...
                    return (
                      <button
                        ...
                        className="px-2 py-0.5 rounded bg-academic-black border border-academic-card/50 hover:border-academic-gold/20 text-[9px] font-mono text-academic-text-muted hover:text-academic-cream transition-colors"
                      >
            ...
            {activeNodeData.type === 'document' && (
              <div className="border-t border-academic-card/40 pt-4 mt-6 flex justify-end">
                <button
                  onClick={() => {
                    const matchedDoc = documents.find(d => d.name === activeNodeData.label);
                    if (matchedDoc) {
          ...
          <div className="bg-academic-paper border border-academic-card p-5 rounded-xl gold-glow">
            ...
            <div className="text-[11px] font-mono text-academic-text-muted space-y-2">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-academic-gold">4 Sectors</span>
              </div>
              <div className="flex justify-between">
